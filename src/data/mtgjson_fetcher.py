...
class MtgJsonFetcher:
    def __init__(self, dataset: str, save_root:str='data/raw/mtgjson'):
        ...
        # final_path = save_root / file_base.stem
    ...
    def _windows_data_fetch(self):
        ...
        os.system(f"powershell Invoke-WebRequest -Uri {self.api_url} -OutFile {self.filepath}")
        os.system(f"powershell Expand-Archive -Path {self.filepath} -DestinationPath {self.final_path}")
        os.system(f"powershell Remove-Item {self.filepath}")
        size = os.system(f"powershell Get-ChildItem {self.save_root} | Measure-Object -Property Length -Sum | ForEach-Object {{ $_.Sum / 1MB }}")
        return size
